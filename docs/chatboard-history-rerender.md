...
## 九、附录：小图标的亚像素偏移 —— send-btn 箭头

> 本节与上面的分页修复是同一次改动里的另一条独立线（同一文件、同一选题），记录一次完整的
> 「现象 → 机制 → 解法」排查。凡是遇到「小图标在容器里看着没居中，微调后却整像素跳过去」，
> 都可以照这套流程看。
...
| 静止时箭头看着偏右 | 量级在半个像素以内 |
| `left: -1px` | 过头（偏左） |
| `left: -0.5px` | **也过头** → 真值落在 0 与 -0.5 之间 |
| 悬停抬起 / 点击飞行动画**进行中** | 箭头是对齐的 |
| 动画**播放完毕** | 立刻又偏右，视觉上有可观测的跳变 |

### 9.2 为什么「应该」是精确居中的

- `.send-btn` 是 flex（`align-items` / `justify-content: center`），居中的是 `.anticon` 这个**盒子**；
- `.anticon` 的尺寸就是里面 svg 的 `1em`，按钮 `font-size: 14px` → 14×14 正方形；按钮 28×28，四周各剩 7px；
- svg 的 `viewBox="64 64 896 896"` 是正方形，`preserveAspectRatio` 默认 `xMidYMid meet` → 等比铺满、不留白；
- 箭头 glyph 在 viewBox 内的留白左右各 92、上下各 88，**两侧相等**。

盒子居中 + 盒子内对称 = 几何上无偏移。所以那半个像素不是布局算错了。

### 9.3 机制：两条光栅化路径

浏览器把 CSS 坐标映射到设备像素时，**独立合成层**与**父层**走的是两条不同的路：

| 路径 | 光栅化 | 位置取整发生在 |
|---|---|---|
| 无独立层（作为父层内容绘制） | 在父层位图里绘制，水平位置直接继承**小数的布局坐标** | 「画进父层位图」这一步 |
| 有独立层 | 以层自己的**局部原点**烘焙成位图，此时不知道层最终落在页面何处 | 合成器把整层**对齐到整数设备像素** |

两条路的取整阶段不同 → 同一个元素可能落在不同的设备像素上。而**合成层是随动画的起止创建/销毁的**：

| 时刻 | 有无独立层 | 位置取整发生在 | 观感 |
|---|---|---|---|
| 静止 | 无 | 画进父层位图时 | 偏右 |
| 悬停抬起过渡进行中 | 有（动画临时提升） | 合成器对齐整层 | 对齐 |
| 过渡结束（transform 值仍挂着） | **被回收** | 回到父层 | 偏右 ← 跳变 |
| 点击飞行中 | 有 | 合成器对齐整层 | 对齐 |
| 飞行结束 | **被回收** | 回到父层 | 偏右 ← 跳变 |

也就是说：**「静止偏右」与「动画结束跳一下」是同一个 bug 的两面**——一面是某条路算错了，另一面是在两条路之间来回切。

### 9.4 为什么不存在「正确的偏移量」

- 动的那一态本来就是对的，偏右只发生在静态；任何静态 `left` 微调都会把**动的那一态**弄坏；
- 静态那条路的落点是**离散**的（被取整到设备像素网格），所以亚像素微调要么无效、要么整像素跳过去——`-0.25px` 那个「甜点」在物理上不存在。

结论：**不要继续调数值，先让两态走同一条路。**

### 9.5 本次解法：让图标常驻合成层

```css
.send-btn :deep(.anticon) {
  will-change: transform; /* 常驻合成层：静止态与动画态走同一条光栅化路径 */
  transition: transform 0.2s ease-out;
}
```

- `will-change` 是给引擎的提示，Blink 收到后**立刻建立并长期保留**该层——这正是该属性的设计目的（把提升开销提前付掉，避免首帧卡顿）；
- **不能用 `transform: translateZ(0)` 代替**：动画/过渡会用自己的 `transform` 把声明的基础值整个替换掉，层提示随之丢失；`will-change` 是独立属性，不会被替换。

**验证手段**：DevTools → 更多工具 → Rendering → **Layer borders**。模型成立的判据是「层边框只在动画期间出现」；加上 `will-change` 后应变成常驻。

### 9.6 这是「绕过」而非「根治」

根因是**图标的水平位置落在小数 CSS 像素上**：`.chat-body` 的 `1fr 3fr` 网格叠上固定 padding 逐层传递，列宽几乎必然带小数，按钮右对齐后又继承了左列右边缘的小数部分。两类根治方向：

| 方向 | 做法 | 代价 / 前提 |
|---|---|---|
| **位置整数化** | 量出可用宽度，把左列宽度显式设成整数。下游间距尺寸本来就全是整数（16 + 16 + 5 + 28 − 7 = 42），左列右边缘一取整，图标位置自动取整 | 需 JS 接管一段布局（`ResizeObserver` + CSS 变量 + `.app-header` / `.chat-body` 两处网格同步） |
| **换绘制原语** | 用字体字形或 border 三角替代 SVG：这类内容就地亚像素抗锯齿、不作整体吸附，位置有无小数都不跳，**改窗口大小也不复发** | 要自己画箭头、边缘略软；仅机制推断，未实测 |

还有个绕不过的约束：**整数化只在 CSS→设备像素为整数倍时成立**（100% / 200% 缩放）；110% 这类缩放下，再整的 CSS 坐标落到设备网格也是小数，取整歧义又回来。反倒是**层方案由合成器按整数设备像素对齐，任意缩放下都有一套确定规则**——三个方向各占一个维度，没有哪个全面压制另外两个。

**本次取舍**：采用层方案（一行、可逆、不需要接管布局）。若该页面以后出现更多位置敏感的亚像素元素（细分割线、1px 描边、小图标），再把左列宽度整数化——那时才是「一处解决所有」，而不是逐个 `will-change`。
